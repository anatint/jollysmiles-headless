...
export default function AboutHero({ data }: { data?: any }) {
  ...
  const heading = item.heroHeading || 'About Us';
  const tagline = item.heroTagline || 'Compassionate Care. Exceptional Results.';
  const body = item.heroBody || "At Jolly Smiles, we're more than just a dental clinic – we're a team that truly cares about your smile and your well-being.";
  const heroImage = getWixImageUrl(item.heroImage, '/about-hero.png');
