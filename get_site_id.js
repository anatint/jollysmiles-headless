...
const { sites } = require('@wix/site-management');
...
async function getSiteId() {
  ...
  try {
    const result = await client.sites.querySites().find();
    console.log("Sites found:", result.items.map(s => ({ id: s.id, name: s.displayName })));
  } catch (error) {
    console.error("Failed to query sites:", error.message);
  }
}
